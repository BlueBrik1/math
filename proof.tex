...
The signed-pair construction admits infinitely many unreduced
representations of the same signed integer.
...
and applying \(S\) exactly \(v\) times to a canonical set \(P_a\)
produces \(P_{a+v}\), we have
...
By the definition of \(R\), whenever both components are nonempty,
one application of \(N\) is made to each component simultaneously.
...
Therefore adding the same canonical contribution to both components
does not change the reduced signed integer represented by the pair.
...
Thus a single signed integer does not have only one possible
...
contains infinitely many canonical sets, there are infinitely many
possible choices of \(P_v\), and therefore infinitely many unreduced
representations of the same signed integer.
...
Independently shift the two input pairs by \(P_v\) and \(P_w\),
respectively.
...
Thus each shifted pair represents exactly the same signed integer
as its corresponding original pair.

The definition of signed addition combines the positive components,
combines the negative components, and then applies \(R\).

Adding \(v\) to both components of the first pair contributes the
same additional amount to the positive and negative sides.

Likewise, adding \(w\) to both components of the second pair
contributes the same additional amount to the positive and negative
sides.

These equal contributions cancel under \(R\).
...
Since \(P_v\) and \(P_w\) may each be chosen independently from the
infinite family
...
a single signed addition may be represented by infinitely many
...
Consequently, the mapping from such input representations to the
reduced signed output is not injective.
...
Nor does it uniquely determine the particular unreduced input pairs
used in the computation.

There are infinitely many distinct admissible input representations
that may correspond to the same signed output.

This is a many-to-one property of the representation system.

It should not by itself be interpreted as a proof of computational
hardness or as a statement concerning the complexity-theoretic
question \(P\) versus \(NP\). Rather, it establishes that information
about the particular input representation is lost when the inputs
are reduced to a single signed result.
...
Hence signed subtraction also admits infinitely many distinct
unreduced input representations that produce the same reduced
signed output.
...
Under the non-unique representation construction, two additional
canonical sets may be chosen:
...
These determine the independent shifts applied to the two signed
input pairs.

The complete representation may therefore depend upon the six
canonical sets
...
while the function ultimately returns a single signed-integer value,
represented canonically by one reduced signed pair.

Thus, when the output is regarded as one signed value, the
construction has a
...
input-set-to-output-value relationship:

\[
\boxed{
6\text{ canonical input sets}
\longrightarrow
...
Therefore the ordinary signed operation is contained as the special
case
...
The additional sets \(P_v\) and \(P_w\) do not alter the resulting
signed value. They alter only the particular unreduced
representation used to obtain that value.

This distinction between the represented value and the particular
representation used to encode that value will be relevant when
considering later applications involving concealed or randomized
representations.
\end{document}